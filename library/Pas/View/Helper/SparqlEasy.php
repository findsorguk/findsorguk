...
class Pas_View_Helper_SparqlEasy extends Zend_View_Helper_Abstract {
	...
	public function sparqlEasy() {
		...
		$this->_endpoint = new EasyRdf_Sparql_Client('http://live.dbpedia.org/sparql');
	...
	public function getSparqlData(){
		...
		. $this->_id . '  .  ?x dbpedia-owl:abstract ?abstract . ?x dbp:place ?place}';
		$key = md5($query);
	...
	public function render()
	{
		...
		foreach($dataSparql as $data){
			$html .='<li>';
			$html .= $this->_wikiLink($data->x) . ' : ' . $this->_cleaner($data->place);
			$html .= '<br />';
			$html .= $data->abstract;
			$html .= '</li>';
		}
