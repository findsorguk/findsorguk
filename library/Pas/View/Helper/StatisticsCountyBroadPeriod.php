 ...
 */
class Pas_View_Helper_StatisticsCountyBroadPeriod extends Zend_View_Helper_Abstract
{
    protected $_solr;
    ...
    public function statisticsCountyBroadPeriod()
    {
        return $this;
    }
...
//    'fields'        => array('*'),
        ...
        foreach ($data as $field) {
            foreach ($field->getFacets() as $field => $facet) {
                foreach ($facet AS $facetStats) {
                    $stats[] = array(
                        'broadperiod' => $facetStats->getValue(),
        ...
        foreach ($stats as $k => $v) {
            $sort['broadperiod'][$k] = $v['broadperiod'];
            $sort['finds'][$k] = $v['finds'];
        }
        array_multisort($sort['broadperiod'], SORT_ASC, $sort['finds'], SORT_ASC, $stats);
...
//	}
    ...
    public function buildHtml($data)
    {
        ...
        $html .= $this->view->partialLoop('partials/annual/objectPeriodsBreakdown.phtml', $data);
