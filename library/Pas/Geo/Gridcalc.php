 ...
 * has been heavily adapted. The UTM functions are adapted
 * from https://gist.github.com/840476
 *
 * An example of use:
 *
 * <code>
...
class Pas_Geo_Gridcalc
{
    ...
    public function convert($unit = 'm', $round = 8)
    {
        ...
        if (!$countCheck) {
            throw new Pas_Geo_Exception('Incorrect grid length', 500);
        }
        ...
        if (!is_numeric($eastcoord)) {
            throw new Pas_Geo_Exception('Easting contains alpha characters' . 500);
        }
        //Confirm numeric for northing coordinates
        if (!is_numeric($northcoord)) {
            throw new Pas_Geo_Exception('Northing contains alpha characters', 500);
        }
        ...
        if ($unit === 'm') {
            ...
            throw new Pas_Geo_Exception('Units are not correct', 500);
        }
        ...
        $geodata = array(
            ...
            'twoPointFiveK' => $twoPointFiveKMap,
            '10kmap' => $tenKMap,
            'easting' => $gridX,
            'northing' => $gridY,
    ...
    private function _stripgrid($string)
    {
        ...
        $gridRefStripped = strtoupper($gridRef);
        return $gridRefStripped;
    }

    /** Function to check if the grid reference is the correct length
     * @access public
     * @param integer $rawcount
     * @return boolean
     */
    public function _countcheck($rawcount)
    {
     ...
     */
    public function _eNtoLat($East, $North, $a, $b, $e0, $n0, $f0, $PHI0, $LAM0)
    {
        ...
        $VIII = ((tan($PHId)) / (24 * $rho * pow($nu, 3))) * (5 + (3
                    * (pow(tan($PHId), 2))) + $eta2
                - (9 * $eta2 * (pow(tan($PHId), 2))));
        $IX = ((tan($PHId)) / (720 * $rho * pow($nu, 5))) * (61 + (90
                    * ((tan($PHId)) ^ 2)) + (45
                    * (pow(tan($PHId), 4))));
        $E_N_to_Lat = (180 / self::PI) * ($PHId - (pow($Et, 2) * $VII)
                + (pow($Et, 4) * $VIII) - (($Et ^ 6) * $IX));
        return ($E_N_to_Lat);
    }
     ...
     * @return integer
     */
    public function _initialLat($North, $n0, $afo, $PHI0, $n, $bfo)
    {
     ...
     * @param $P2
     */
    public function _marc($bFo, $n, $P1, $P2)
    {
        $n2 = $n * $n;
        $n3 = $n * $n * $n;
        $Marc = $bFo * (((1 + $n + ((5 / 4) * ($n2)) + ((5 / 4) * ($n3)))
                    * ($P2 - $P1)) - (((3 * $n) + (3 * ($n2))
                        + ((21 / 8) * ($n3))) * (Sin($P2 - $P1))
                    * (Cos($P2 + $P1))) + ((((15 / 8) * ($n2)) + ((15 / 8) * ($n3)))
                    * (Sin(2 * ($P2 - $P1))) * (Cos(2 * ($P2 + $P1))))
    ...
    private function _eNtoLong($East, $North, $a, $b, $e0, $n0, $f0, $PHI0, $LAM0)
    {
        ...
        //Compute Longitude
        $X = (pow(cos($PHId), -1)) / $nu;
        $XI = ((pow(cos($PHId), -1)) / (6 * pow($nu, 3))) * (($nu / $rho)
                + (2 * (pow(tan($PHId), 2))));
        $XII = ((pow(cos($PHId), -1)) / (120 * pow($nu, 5))) * (5 + (28
                    * (pow(tan($PHId), 2)))
                + (24 * (pow(tan($PHId), 4))));
        $XIIA = ((pow(Cos($PHId), -1)) / (5040 * pow($nu, 7))) * (61 + (662
                    * (pow(tan($PHId), 2)))
                + (1320 * (pow(Tan($PHId), 4))) + (720 * (pow(tan($PHId), 6))));
        $E_N_to_Long = (180 / self::PI) * ($RadLAM0 + ($Et * $X) - (pow($Et, 3)
                    * $XI) + (pow($Et, 5) * $XII)
                - (pow($Et, 7) * $XIIA));
        return $E_N_to_Long;
    }
     ...
     * @return integer
     */
    public function _latLongHtoX($PHI, $LAM, $H, $a, $b)
    {
        //Convert angle measures to radians
     ...
     * @return integer
     */
    public function _latLongHtoY($PHI, $LAM, $H, $a, $b)
    {
        // Convert angle measures to radians
        $RadPHI = $PHI * (self::PI / 180);
        $RadLAM = $LAM * (self::PI / 180);
     ...
     * @return integer
     */
    public function _latHtoZ($PHI, $H, $a, $b)
    {
        // Convert angle measures to radians
        $RadPHI = $PHI * (self::PI / 180);
        // Compute eccentricity squared and nu
     ...
     * @return integer
     */
    public function _helmertX($X, $Y, $Z, $DX, $Y_Rot, $Z_Rot, $s)
    {
        $sfactor = $s * 0.000001;
     ...
     * @param $s
     */
    public function _helmertY($X, $Y, $Z, $DY, $X_Rot, $Z_Rot, $s)
    {
        $sfactor = $s * 0.000001;
     ...
     * @return integer
     */
    public function _helmertZ($X, $Y, $Z, $DZ, $X_Rot, $Y_Rot, $s)
    {
        $sfactor = $s * 0.000001;
        $RadX_Rot = ($X_Rot / 3600) * (self::PI / 180);
        $RadY_Rot = ($Y_Rot / 3600) * (self::PI / 180);
     ...
     * @return integer
     */
    public function _xyzToLat($X, $Y, $Z, $a, $b)
    {
     ...
     * @return integer
     */
    public function _xyzToLong($X, $Y)
    {
    ...
    public function fourFigure($digits = 4)
    {
        ...
        $cleanLetterPair = strtoupper($letterpair); //transform lowercase to upper
        ...
        return $cleanLetterPair . $eastcoord . $northcoord;
    }
     ...
     */
    public function _decimalToOrdinalCoords($lat, $lon)
    {
        $ordinalNS = ($lat > 0) ? "N" : "S";
        $ordinalEW = ($lon > 0) ? "E" : "W";
        $deg = "&amp;deg;";
        ...
        $degLatValue = $latDeg . $deg . floor($latMin) . $min . floor($latSec)
            . $sec . $ordinalNS;
        $lonDeg = ($lon > 0) ? abs(floor($lon)) : abs(ceil($lon));
        $lonMin = abs(($lon - floor($lon)) * 60);
        $lonSec = abs(($lonMin - floor($lonMin)) * 60);
        $degLonValue = $lonDeg . $deg . floor($lonMin) . $min . floor($lonSec)
            . $sec . $ordinalEW;
        $degreedData = array(
            'degreesLat' => $latDeg,
     ...
     */
    public function _convertLatLonUtm($lat, $lon, $longOrigin = null)
    {
        $k0 = 0.9996;
        $falseEasting = 0.0;
        ...
        $M = $this->_a * ((1 - $this->_e2 / 4 - 3 * $this->_e2 * $this->_e2 / 64 - 5
                    * $this->_e2 * $this->_e2 * $this->_e2 / 256)
                * $latRad - (3 * $this->_e2 / 8 + 3 * $this->_e2
                    * $this->_e2 / 32 + 45 * $this->_e2 * $this->_e2
                    * $this->_e2 / 1024) * sin(2 * $latRad)
                + (15 * $this->_e2 * $this->_e2 / 256 + 45 * $this->_e2
                    * $this->_e2 * $this->_e2 / 1024) * sin(4 * $latRad)
                - (35 * $this->_e2 * $this->_e2 * $this->_e2 / 3072)
                * sin(6 * $latRad));
        ...
        $utmNorthing = ($k0 * ($M + $N * tan($latRad) * ($A * $A / 2 + (5
                        - $T + 9 * $C + 4 * $C * $C) * $A * $A * $A
                    * $A / 24 + (61 - 58 * $T + $T * $T + 600 * $C - 330
    ...
    private function _utmLetterDesignator($lat)
    {
        if ((84 >= $lat) && ($lat >= 72)) {
            ...
            throw new Pas_Geo_Exception('This UTM is outside the limits');
        }
     ...
     * @return integer
     */
    public function _xyzToH($X, $Y, $Z, $a, $b)
    {
     ...
     * @return integer
     */
    public function _latLongToEast($PHI, $LAM, $a, $b, $e0, $f0, $PHI0, $LAM0)
    {
        $RadPHI = $PHI * (self::PI / 180);
        $RadLAM = $LAM * (self::PI / 180);
        $RadPHI0 = $PHI0 * (self::PI / 180);
        ...
        $p = $RadLAM - $RadLAM0;
        $IV = $nu * (cos($RadPHI));
        $V = ($nu / 6) * (pow(cos($RadPHI), 3)) * (($nu / $rho)
                - (pow(tan($RadPHI), 2)));
        $VI = ($nu / 120) * (pow(cos($RadPHI), 5)) * (5 - (18
                    * (pow(tan($RadPHI), 2))) + (pow(tan($RadPHI), 4))
     ...
     * @return integer
     */
    public function _latLongToNorth($PHI, $LAM, $a, $b, $e0, $n0, $f0, $PHI0, $LAM0)
    {
        $RadPHI = $PHI * (self::PI / 180);
        $RadLAM = $LAM * (self::PI / 180);
        $RadPHI0 = $PHI0 * (self::PI / 180);
        ...
        $III = (($nu / 24) * (sin($RadPHI)) * (pow(cos($RadPHI), 3)))
            * (5 - (pow(tan($RadPHI), 2)) + (9 * $eta2));
        $IIIA = (($nu / 720) * (sin($RadPHI)) * (pow(cos($RadPHI), 5)))
            * (61 - (58 * (pow(tan($RadPHI), 2)))
                + (pow(tan($RadPHI), 4)));
     ...
     * @param $bFo
     */
    public function _phid($gridX, $N0, $aFo, $PHI0, $n, $bFo)
    {
