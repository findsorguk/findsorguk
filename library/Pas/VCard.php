...
class Pas_VCard
{
    ...
    public function buildCard(array $data)
    {
        ...
        $card .= 'REV:' . $this->getRevisionDate() . PHP_EOL;
        if (array_key_exists('display_name', $data)) {
...
//                $photo = base64_encode(file_get_contents($data['photo']));
     ...
     */
    public function createHeaders($filename)
    {
        $ext = '.vcf';
        $front = Zend_Controller_Front::getInstance()->getResponse();
        $front->setHeader('Content-type', 'text/directory');
        $front->setHeader('Content-Disposition', 'attachment; filename=' . $filename . $ext);
        $front->setHeader('Pragma', 'public');
        return;
    }
}