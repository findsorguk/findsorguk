...
class Pas_Calendar_Mapper  {
    ...
    public function _setTonight($tonight) {
        if(isset($tonight)){
            $this->_tonight = date('Y-m-d' 
                    . strtotime($this->_today * 24*60*60)) 
                    . $this->_timezone;
        } else {
    ...
    public function getEventFeed(  
            ...
            $orderBy = 'starttime', $startTime = NULL, $endTime = NULL, 
            $timeZone = NULL, $fullTextQuery = NULL  ) {
        $query = $this->getCalendar()->newEventQuery();
        //Preset options
        $query->setUser( $user );
    ...
    public function addEvent(array $data) {
        if(is_array($data)){
            ...
            $properties = array(
                'type' => $data['eventType'], 'creator' => $data['creator']
                    );
