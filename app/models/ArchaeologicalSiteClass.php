<?php
/**
 * A model for accessing archaeological site classes from database
 *
 * An example of use:
 *
 * <code>
...
class ArchaeologicalSiteClass extends Pas_Db_Table_Abstract {

    /** The table name
     * @access protected
     * @var string
     */
    protected $_name = 'archsiteclass';
    /** The primary key
     * @access protected
     * @var integer
     */
    ...
    public function getOptions() {
        $key = md5('archsiteclassss');
        if (!$options = $this->_cache->load($key)) {
            $select = $this->select()
                ->from($this->_name, array('id', 'siteclass'))
                ->order('siteclass ASC')
                ->where('valid = ?', (int)1);
