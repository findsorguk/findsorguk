...
class EditAccountForm extends Pas_Form
{
    ...
    public function init()
    {
        ...
        $preferred_name = $this->addElement(
            'text',
            'preferred_name',
        ...
        $institution = $this->addElement(
            'select',
            'institution',
            array('label' => 'Recording institution:')
        )->institution;
        $institution->addMultiOptions(
            array(
        ...
        $canRecord = $this->addElement(
            'checkbox',
            'canRecord',
            array('label' => 'Allowed to record: ')
        )->canRecord;

        $role = $this->addElement(
            'select',
            'role',
            array('label' => 'Site role: ')
        )->role;
        ...
        $person = $this->addElement(
            'text',
            'person',
            array('label' => 'Personal details attached: ')
        )->person;
        $peopleID = $this->addElement('hidden', 'peopleID', array())->peopleID;

        $valid = $this->addElement(
            'checkbox',
            'valid',
            array('label' => 'Valid record: ')
        )->valid;

        $activationKey = $this->addElement(
            'text',
            'activationKey',
            array('label' => 'Activation key: ')
        )->activationKey;
