	...
	if (GBrowserIsCompatible()) {
		...
		switch (options.mapControl) {
			case "small":
				...
				break;
			case "smallzoom":
				thisMap.addControl(new GSmallZoomControl());
				break;
			case "large":
		...
		switch (options.mapEnableType)
		{
		case "menumap":
		thisMap.addControl(new GMenuMapTypeControl());
		break;
		case true:
		thisMap.addControl (new GMapTypeControl());
		break;
		case "hierarch":
		thisMap.addControl (new GHierarchicalMapTypeControl());
		break;
		};
		
		switch (options.mapAddTerrainType)
		{
		case true:
	    thisMap.addMapType(G_PHYSICAL_MAP);
		break;
		};
		
		// Type of map Control (Map,Sat,Hyb)
	//	if (options.mapEnableType) 
		//	thisMap.addControl(new GMenuMapTypeControl()); // Off by default
			//thisMap.addMapType(G_PHYSICAL_MAP);
